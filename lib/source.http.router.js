...
const PATH_EXPORT = '/alertsources/export';
...
router.get(PATH_LIST, function getAlertSources(request, response, next) {
  ...
  AlertSource.get(options, function onGetAlertSources(error, results) {
    // forward error
    if (error) {
      next(error);
    }
...
/**
 * @api {get} /alertsources/export Export AlertSources
 * @apiVersion 1.0.0
 * @apiName ExportAlertSources
 * @apiGroup AlertSource
 * @apiDescription Export alert sources as csv
 * @apiUse RequestHeaders
 */
router.get(PATH_EXPORT, function getExport(request, response) {
  ...
  response.attachment(`alert_sources_exports_${Date.now()}.csv`);
  response.status(200);
  AlertSource.exportCsv(options).pipe(response);
});
...
router.post(PATH_LIST, function postAlertSource(request, response, next) {
  ...
  AlertSource.post(body, function onPostAlertSource(error, created) {
    // forward error
    if (error) {
      next(error);
    }
...
router.get(PATH_SINGLE, function getAlertSource(request, response, next) {
  ...
  AlertSource.getById(options, function onGetAlertSource(error, found) {
    // forward error
    if (error) {
      next(error);
    }
...
router.patch(PATH_SINGLE, function patchAlertSource(request, response, next) {
  ...
  AlertSource.patch(id, patches, function onPatchAlertSource(error, patched) {
    // forward error
    if (error) {
      next(error);
    }
...
router.put(PATH_SINGLE, function putAlertSource(request, response, next) {
  ...
  AlertSource.put(id, updates, function onPutAlertSource(error, updated) {
    // forward error
    if (error) {
      next(error);
    }
