 ...
 * differentiate an alert visually.
...
/* declarations */
const Alert = include(__dirname, 'alert.model');
...
/**
 * @api {get} /alerts/schema Get Alerts Schema
 * @apiVersion 1.0.0
 * @apiName GetAlertSchema
 * @apiGroup Alert
 * @apiDescription Returns alert json schema definition
 * @apiUse RequestHeaders
 */
router.get(PATH_SCHEMA, function getAlertSchema(request, response) {
  const schema = Alert.jsonSchema();
  response.status(200);
  response.json(schema);
});

/**
 * @api {get} /alerts/export Export Alerts
 * @apiVersion 1.0.0
 * @apiName ExportAlerts
 * @apiGroup Alert
 * @apiDescription Export alerts as csv
 * @apiUse RequestHeaders
 */
router.get(PATH_EXPORT, function getExport(request, response) {
  ...
  response.attachment(`alerts_exports_${Date.now()}.csv`);
  response.status(200);
  Alert.exportCsv(options).pipe(response);
});
...
router.post(PATH_LIST, function postAlert(request, response, next) {
  ...
  Alert.post(body, function onPostAlert(error, created) {
    //forward error
    if (error) {
      next(error);
    }
