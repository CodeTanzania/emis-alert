...
const { centroidOf } = require('mongoose-geojson-schemas');
...
AlertSchema.pre('validate', function (next) {
  ...
  if (this.area && this.area.geometry) {
    const centroid = centroidOf(this.area.geometry);
    if (centroid) {
      this.area.centroid = centroid;
    }
  }
